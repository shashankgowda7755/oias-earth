...
A faithful, cleanly-restructured rebuild of the **"COMMUNITREE" /
...
| Separate auth host (`dev-auth.communitree.co.in`) | `POST /api/v1/auth/login` on the same Express server, signing a JWT with the same secret the REST middleware verifies. Self-contained. |
