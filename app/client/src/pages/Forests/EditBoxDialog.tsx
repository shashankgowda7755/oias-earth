...
export function EditBoxDialog({
  ...
  const hasPrefix = draft.prefix.trim().length > 0;
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <TextField
              ...
              type="number"
              inputMode="numeric"
              value={draft.start_digits}
              onChange={(v) => setField('start_digits', v)}
              helperText="Zero-pad width"
            />
            <TextField
              label="Start"
              type="number"
          ...
          <div className="mt-5">
            {!hasPrefix ? (
              <p className="rounded-card border border-dashed border-border bg-appbg px-3 py-4 text-center text-sm text-textSecondary">
                Enter prefix to add species details.
              </p>
            ) : (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-textPrimary">Species</h3>
                  {overCapacity ? (
                    <span className="text-label text-danger">
                      Planted exceeds capacity by {Math.abs(remaining)}.
                    </span>
                  ) : null}
                </div>
                ...
                {draft.species.length === 0 ? (
                  <p className="text-label text-textSecondary">
                    No species yet. Add a species and its tree count.
                  </p>
                ) : (
                ...
                <Button variant="outlined" onClick={addSpeciesRow} className="text-sm">
                  + Add species
                </Button>
              </div>
            )}
